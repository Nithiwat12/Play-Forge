...
export function ItoGame({ room, publicState, privateState, selfUserId, onAction }: GameComponentProps) {
  ...
  return <div className="mx-auto max-w-5xl space-y-5 pb-8 text-slate-100">
    ...
    {s.phase === "PLAY" && <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
      <Card><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">ไพ่ลับของคุณ</h2><Button variant="secondary" onClick={() => setShown(!shown)}>{shown ? "ซ่อนเลข" : "ดูเลขของฉัน"}</Button></div><p className="mt-2 text-xs text-slate-400">อย่าให้คนอื่นเห็นหน้าจอตอนเปิดเลข</p><div className="mt-4 space-y-4">{mine.cards.map(c => <div key={c.id} className="rounded-xl border border-teal-800 bg-teal-950/30 p-4"><p className="mb-3 text-center text-5xl font-bold text-teal-200">{shown ? c.value : "?"}</p>{s.mode === "ONLINE" ? <><label className="text-sm" htmlFor={c.id}>คำใบ้ไพ่ใบนี้</label><input id={c.id} className="my-2 w-full rounded-lg border border-slate-600 bg-slate-900 p-3" maxLength={160} disabled={!!proposal || busy} value={drafts[c.id] ?? c.clue} placeholder="ยกตัวอย่างให้ตรงกับระดับของเลข" onChange={e => setDrafts({ ...drafts, [c.id]: e.target.value })} /></> : <p className="mb-3 rounded-lg bg-teal-950/50 p-3 text-sm text-teal-200">พูดคำใบ้ตามหัวข้อให้เพื่อนฟัง อย่าบอกเลข แล้วเสนอเปิดเมื่อคิดว่าใบนี้ต่ำที่สุด</p>}<div className="flex flex-wrap gap-2">{s.mode === "ONLINE" && <Button variant="secondary" disabled={busy || !!proposal || !(drafts[c.id] ?? c.clue).trim()} onClick={() => act("clue", { cardId: c.id, text: drafts[c.id] ?? c.clue })}>ส่งคำใบ้</Button>}<Button disabled={blocked || !!proposal} onClick={() => act("propose", { cardId: c.id })}>เสนอเปิดใบนี้</Button></div>{c.clue && <p className="mt-2 text-xs text-teal-300">เผยแพร่แล้ว: {c.clue}</p>}</div>)}{!mine.cards.length && <p className="text-slate-400">เปิดไพ่ครบแล้ว ช่วยเพื่อนยืนยันไพ่ที่เหลือได้</p>}</div></Card>
      <Card><h2 className="font-semibold">{s.mode === "ONLINE" ? "คำใบ้ของทุกคน" : "เพื่อนร่วมวง · ฟังคำใบ้แล้วตัดสินใจ"}</h2><div className="mt-4 space-y-3">{s.players.map(p => <div key={p.userId} className="rounded-xl bg-slate-900 p-3"><p className="text-sm font-semibold text-teal-300">{p.username} {p.userId === selfUserId && "(คุณ)"} · {p.cards.length} ใบ</p>{p.cards.map((c, i) => <p key={c.id} className="mt-2 break-words text-sm">ใบ {i + 1}: {c.clue || (s.mode === "TABLE" ? "ฟังคำใบ้จากเพื่อน" : "กำลังคิดคำใบ้…")}</p>)}</div>)}</div></Card>
    </div>}
