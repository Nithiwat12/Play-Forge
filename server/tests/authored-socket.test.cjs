...
for(const slug of ['wordhead'])for(const mode of ['TABLE','ONLINE'])test(`Socket.IO authored ${mode}: private submission, assignment and reconnect`,async()=>{
 const n=3;sessions.length=0;room={id:`${slug}-${mode}`,roomCode:'MODE12',roomName:'Modes test',createdAt:new Date(),status:'WAITING',hostId:'p1',gameId:slug,settings:{playMode:mode,wordSource:"PLAYERS"},maxPlayers:n,game:{slug,name:slug,minPlayers:3,maxPlayers:8},players:Array.from({length:n},(_,i)=>({id:`rp${i+1}`,userId:`p${i+1}`,user:{username:`Player${i+1}`},joinedAt:new Date(),isReady:true,isHost:i===0}))};
 ...
 assert.equal(g.getPublicState().phase,'SUBMIT_WORDS');
 for(let i=0;i<n;i++){
 const word=`โจทย์จากผู้เล่น${i+1}`;
 assert.equal((await send(`p${i+1}`,'submitWord',{word})).ok,true);
 if(i<n-1){assert.equal(JSON.stringify(g.getPublicState()).includes(word),false);for(let j=0;j<n;j++)if(i!==j)assert.equal(JSON.stringify(g.getPrivateState(`p${j+1}`)).includes(word),false);}
 }
 assert.equal(g.getPublicState().phase,'TURN');
 for(let i=0;i<n;i++)assert.notEqual(g.assignedWords.get(`p${i+1}`),`โจทย์จากผู้เล่น${i+1}`);
 if(slug==='spyfall'){
